...
/// Loads the vendor certificate chains into a hash map, with their processor type as a key and returns a static reference to it.
...
// pub(crate) fn load_cert_chain() -> &'static Result<HashMap<ProcType, VendorCertificates>> {
//     static CERT_CHAINS: OnceLock<Result<HashMap<ProcType, VendorCertificates>>> = OnceLock::new();
//     CERT_CHAINS.get_or_init(|| {
//         let mut map = HashMap::new();
//         for proc in ProcType::iter() {
//             let cert_authority = match proc {
//                 ProcType::Milan => include_bytes!("milan_ask_ark_asvk.pem"),
...
//             let certs = X509::stack_from_pem(cert_authority).context("Failed to parse PEM file")?;
...
impl Verifier for Snp {
    ...
    async fn evaluate(
        ...
        let SnpEvidence {
            ...
            cert_chain,
        } = serde_json::from_slice(evidence).context(format!(
            "Deserialize Quote failed. Evidence {}",
        ...
        let proc_model: ProcType = get_processor_model(&report)?;

        // Get vendor certs for specific processor type
        let vendor_certs = CERT_CHAINS
            .get(&proc_model)
            .ok_or_else(|| anyhow!("Vendor certs not found for processor type: {proc_model:?}"))?;
        ...
        let vek = match cert_chain {
            ...
            _ => {
                // Get VCEK from KDS
                let vcek_buf = fetch_vcek_from_kds(report, proc_model.clone())
                    .await
        ...
        verify_report_tcb(&report, vek, proc_model).context("Reported TCB values do not match")?;
...
async fn fetch_vcek_from_kds(
    ...
    let vcek_url: String = match proc_type {
        ProcType::Turin => {
...
fn get_processor_model(att_report: &AttestationReport) -> Result<ProcType> {
...
mod tests {
    ...
    fn check_milan_certificates() {
        let VendorCertificates { ask, ark, asvk } = CERT_CHAINS.get(&ProcType::Milan).unwrap();
    ...
    fn check_vcek_signature_verification() {
        ...
        let VendorCertificates { ask, ark, asvk: _ } = CERT_CHAINS.get(&ProcType::Milan).unwrap();
        let chain = Chain {
            ca: CaChain {
                ark: ark.clone(),
                ask: ask.clone(),
    ...
    fn check_vlek_signature_verification() {
        ...
        let VendorCertificates { ask: _, ark, asvk } = CERT_CHAINS.get(&ProcType::Milan).unwrap();
        let chain = Chain {
            ca: CaChain {
                ark: ark.clone(),
                ask: asvk.clone(),
    ...
    fn check_report_signature() {
        ...
        verify_report_tcb(&attestation_report, vcek, ProcType::Milan).unwrap();
    }
    ...
    fn check_vlek_report_signature() {
        ...
        verify_report_tcb(&attestation_report, vlek, ProcType::Milan).unwrap();
    }
    ...
    fn check_report_tcb_failure() {
        ...
        let attestation_report = AttestationReport::from_bytes(&bytes).unwrap();
        let vcek = Certificate::from_bytes(VCEK).unwrap();
        verify_report_tcb(&attestation_report, vcek, ProcType::Milan).unwrap_err();
    }
    ...
    fn check_vlek_report_tcb_failure() {
        ...
        let attestation_report = AttestationReport::from_bytes(&bytes).unwrap();
        let vlek = Certificate::from_bytes(VLEK).unwrap();
        verify_report_tcb(&attestation_report, vlek, ProcType::Milan).unwrap_err();
    }
}
